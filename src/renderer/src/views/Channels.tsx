...
const CATALOG: Cat[] = [
  { kind: "slack", label: "Slack", desc: "Mentions, DMs, buttons.", auth: "connect", service: "slack", color: "#611f69" },
  { kind: "discord", label: "Discord", desc: "Slash commands, components.", auth: "env", env: ["DISCORD_PUBLIC_KEY", "DISCORD_APPLICATION_ID", "DISCORD_BOT_TOKEN"], color: "#5865F2" },
  { kind: "teams", label: "Microsoft Teams", desc: "Messages + Adaptive Cards.", auth: "env", env: ["MICROSOFT_APP_ID"], color: "#4b53bc" },
  { kind: "telegram", label: "Telegram", desc: "Bot messages.", auth: "env", env: ["TELEGRAM_BOT_TOKEN", "TELEGRAM_WEBHOOK_SECRET_TOKEN"], color: "#229ED9" },
  { kind: "twilio", label: "Twilio", desc: "SMS & speech phone calls.", auth: "env", env: ["TWILIO_ACCOUNT_SID", "TWILIO_AUTH_TOKEN"], color: "#F22F46" },
  { kind: "github", label: "GitHub", desc: "@mentions, PR review w/ checkout.", auth: "connect", service: "github", color: "#24292f" },
  { kind: "linear", label: "Linear", desc: "Issue delegation, Agent Sessions.", auth: "connect", service: "linear", color: "#5E6AD2" },
  { kind: "web", label: "Web Chat", desc: "A Next.js browser chat app.", auth: "web", color: "#000000" },
...
function AddChannelModal({
  ...
  const [done, setDone] = useState<{ envVars?: string[]; connect?: boolean } | null>(null);
  ...
  const attach = async (): Promise<void> => {
    setAttaching(true);
    setAttachOut(`$ vercel connect attach ${connector} --triggers --trigger-path /eve/v1/${cat.kind}\n`);
    const r = await window.studio.vercel.connectorAttach(agentId, connector, cat.kind);
    setAttaching(false);
    setAttachOut((o) => o + r.output);
  };

  const canAdd =
    cat.auth === "connect" ? Boolean(connector) : cat.kind === "custom" ? Boolean(customName) : true;
      ...
      {done ? (
        <div className="space-y-3 p-4">
          <div className="rounded-lg bg-success/10 px-3 py-2 text-[13px] text-success">
            {cat.kind === "web" ? "Web Chat app added." : `channels/${cat.kind}.ts written.`}
          </div>
          {done.envVars && done.envVars.length > 0 ? (
            <div className="space-y-1.5">
              <p className="text-2xs text-muted">Set these in the Environment tab, then restart:</p>
              <div className="flex flex-wrap gap-1.5">
                {done.envVars.map((v) => (
                  <code key={v} className="rounded bg-black/[0.05] px-1.5 py-0.5 font-mono text-2xs text-text">
                    {v}
                  </code>
                ))}
              </div>
          ...
          {done.connect ? (
            <div className="space-y-2">
              <p className="text-2xs leading-relaxed text-muted">
                ...
                events to <span className="font-mono text-text">/eve/v1/{cat.kind}</span>,
                then deploy.
              </p>
        ...
        <div className="space-y-3 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[14px] font-medium text-text">{cat.label}</div>
          ...
          {cat.auth === "connect" ? (
            <div>
              <div className="mb-1 text-xs font-medium text-muted">Vercel Connect connector</div>
              <ConnectorPicker
                agentId={agentId}
                service={cat.service}
                value={connector}
          ...
          {cat.auth === "env" ? (
            <div className="rounded-lg border border-border bg-subtle p-3 text-2xs leading-relaxed text-muted">
              ...
              {cat.env?.map((v, i) => (
                <span key={v}>
                  {i > 0 ? ", " : ""}
                  <code className="rounded bg-black/[0.05] px-1 font-mono text-text">{v}</code>
                </span>
              ))}{" "}
          ...
          {cat.auth === "web" ? (
            <div className="rounded-lg border border-border bg-subtle p-3 text-2xs leading-relaxed text-muted">
              Runs <span className="font-mono text-text">eve channels add web</span> to
              scaffold a Next.js Web Chat app in the project.
            </div>
          ) : null}
          ...
          {cat.kind === "custom" ? (
            <div>
              <div className="mb-1 text-xs font-medium text-muted">
                File name <span className="text-faint">— channels/&lt;name&gt;.ts</span>
              </div>
              <input
                value={customName}
